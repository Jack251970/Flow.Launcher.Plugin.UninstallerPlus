...
public class BuildContext(ICakeContext context) : FrostingContext(context)
{
    ...
    public const string SlnFile = "../src/Flow.Launcher.Plugin.UninstallerPlus.sln";
...
public sealed class BuildTask : FrostingTask<BuildContext>
{
    ...
    {
        var projects = context.DefaultSln.Value.Projects.Where(p => p.Name.EndsWith("UninstallerPlus"));
...
public class PublishTask : FrostingTask<BuildContext>
{
    public override void Run(BuildContext context)
    {
        var project = context.DefaultSln.Value.Projects.First(p => p.Name.EndsWith("UninstallerPlus"));
        ...
        context.ZipCompress(
            rootPath: srcDir,
            outputPath: $"{dstDir}/Uninstaller+-{context.PublishVersion}.zip",
            filePaths: files,
            level: 9
        );
    }
...
public class DeployTask : FrostingTask<BuildContext>
{
    public override async void Run(BuildContext context)
    {
        ...
        var files = context.GetFiles($"{distDir}/UninstallerPlus*.zip");
        ...
        var oldFiles = context.GetFiles($"{pluginsDir}/UninstallerPlus*");
